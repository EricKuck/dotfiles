// The denial audit log: Seatbelt's `(debug deny)` violations, streamed out of
// the machine-wide macOS unified log and aggregated into one shared SQLite
// database. Every broker starts it; exactly one of them wins the writer
// election and does the streaming.
...
pub type Logger = DenialLogger;

pub fn start() -> Result<Logger, String> {
    start_denial_logger(&denial_log_path())
}
