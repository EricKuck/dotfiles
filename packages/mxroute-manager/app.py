...
class MxRouteAPI:
    def __init__(self, server, username, api_key):
        self.base_url = f"https://{server}/api"
        if server == "api.mxroute.com":
    ...
    def add_forwarder(self, domain, alias, forward_to):
        url = f"{self.base_url}/domains/{domain}/forwarders"
        data = {"alias": alias, "forward_to": forward_to, "json": "yes"}
        try:
    ...
    for domain_name in ALLOWED_DOMAINS:
        ...
        if isinstance(fwds, list):
            for f in fwds:
                ...
                f["full_email"] = f"{f.get('forwarder')}@{domain_name}"
                all_forwarders.append(f)
        elif isinstance(fwds, dict):
            for alias, details in fwds.items():
                dest = (
                    details
                    if isinstance(details, str)
                    else details.get("destination", str(details))
                )
                all_forwarders.append(
                    {
                        ...
                        "destination": dest,
                        "domain": domain_name,
                        "full_email": f"{alias}@{domain_name}",
                    }
                )

    return render_template(
        "index.html",
        forwarders=all_forwarders,
